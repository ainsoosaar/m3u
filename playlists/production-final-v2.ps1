# ============================================
# production-final-v2.ps1 - С ИСПРАВЛЕННЫМ BUILD
# ============================================
...
# ШАГ 2: Используем исправленный build_pokaz.js
Write-Log "🔧 ШАГ 2: Подготовка скрипта сборки..." "Cyan"
$buildScript = Join-Path $playlistsPath "build_pokaz_fixed.js"

if (-not (Test-Path $buildScript)) {
    Write-Log "⚠️ Исправленный скрипт не найден, используем оригинальный" "Yellow"
    $buildScript = Join-Path $playlistsPath "build_pokaz.js"
} else {
    Write-Log "✅ Использование исправленного скрипта: build_pokaz_fixed.js" "Green"
}

# ШАГ 3: Создаем error_log.txt если его нет
$errorLogPath = Join-Path $playlistsPath "error_log.txt"
if (-not (Test-Path $errorLogPath)) {
    "" | Out-File -FilePath $errorLogPath -Encoding UTF8
    Write-Log "✅ Создан файл error_log.txt" "Green"
}

# ШАГ 4: Сборка плейлиста
Write-Log "📦 ШАГ 4: Сборка плейлиста..." "Cyan"
try {
...
# ШАГ 5: Проверка файла плейлиста
Write-Log "📁 ШАГ 5: Проверка файла..." "Cyan"
...
# ШАГ 6: Получение SHA с GitHub
Write-Log "🔑 ШАГ 6: Получение SHA..." "Cyan"
...
# ШАГ 7: Кодирование в Base64
Write-Log "🔐 ШАГ 7: Кодирование в Base64..." "Cyan"
try {
...
Write-Log "📤 ШАГ 8: Отправка на GitHub..." "Cyan"
